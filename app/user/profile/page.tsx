...
const validateNigerianPhone = (phone: string): { isValid: boolean; message: string } => {
  ...
  if (!nigerianPhoneRegex.test(cleaned)) return {
    isValid: false,
...
export default function UserProfile() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const { data: seasonData } = await supabase
        .from('seasons').select('id, application_end_date').eq('status', 'active').limit(1).single()
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isApplicationClosed) { toast.error('Cannot update photo after application deadline'); return }
    ...
    try {
      ...
      const { error: uploadError } = await supabase.storage.from('applications').upload(filePath, file, { upsert: true })
      ...
      const { error: updateError } = await supabase.from('applications').update({ photo_url: photoUrl }).eq('id', application.id)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!phoneValidation.isValid) { toast.error(phoneValidation.message); setPhoneError(phoneValidation.message); return }
      if (!formData.state) { toast.error('State is required'); return }

      if (formData.emergency_phone.trim()) {
        const epv = validateNigerianPhone(formData.emergency_phone)
        if (!epv.isValid) { toast.error(epv.message); setEmergencyPhoneError(epv.message); return }
      }
      ...
      if (userData) { setUser(userData); setPreferredName(userData.preferred_name || '') }
  ...
  const handleUpdateNickname = async () => {
    if (!user) return
    if (!preferredName.trim()) { toast.error('Please enter a nickname'); return }
            ...
            <div className="flex items-start justify-between mb-4 md:mb-6">
              ...
              {isEditing && (
                <label className="cursor-pointer">
                  <input type="file" accept="image/*" onChange={handlePhotoUpload} disabled={uploadingPhoto || isApplicationClosed} className="hidden" />
                  <span className={`inline-flex items-center justify-center w-10 h-10 rounded-lg transition ${
                    ...
                    {uploadingPhoto
                      ? <div className="animate-spin w-5 h-5 border-2 border-naija-green-600 border-t-transparent rounded-full" />
                      : <Camera size={20} />}
                  </span>
                </label>
              )}
            ...
            <div className="flex flex-col md:flex-row gap-6 md:gap-8">
              <div className="relative w-full max-w-[200px] h-56 md:w-56 md:h-64 rounded-lg overflow-hidden bg-gray-200 flex-shrink-0 mx-auto md:mx-0 border-2 border-gray-300">
                {application?.photo_url ? (
                  <Image src={application.photo_url} alt={user?.full_name || 'Profile'} fill className="object-cover" />
                ) : (
                  ...
                  <div className={`${isEditing ? 'border-t pt-4' : ''} ${canEditNickname ? 'border-naija-green-200' : 'border-gray-200'}`}>
                    ...
                    {isEditing ? (
                      ...
                      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 md:p-4">
                        <p className="text-sm md:text-base font-semibold text-gray-900">{preferredName || 'Not set'}</p>
            ...
            <div className="space-y-3 md:space-y-4">
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-200 bg-gray-50">{formData.full_name || 'Not set'}</div>
                )}
              </div>
              ...
              <div>
                <label className="text-xs md:text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2">
                  <Phone size={16} /> Phone Number (WhatsApp Preferred)
                </label>
                {isEditing ? (
                  <>
                    <input type="tel" name="phone" value={formData.phone} onChange={handleChange} placeholder="e.g., 08012345678 or +2348012345678"
                    ...
                    {phoneError && <p className="text-xs text-red-600 mt-1 flex items-center gap-1"><AlertCircle size={12} />{phoneError}</p>}
                  </>
                ) : (
                  <div className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-200 bg-gray-50">{formData.phone || 'Not set'}</div>
                )}
              </div>
                ...
                {isEditing ? (
                  <>
                    <input type="date" name="birth_date" value={formData.birth_date} onChange={handleChange}
                      className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-300 focus:outline-none focus:border-naija-green-600 focus:ring-2 focus:ring-naija-green-100" />
                    {formData.age && <p className="text-xs md:text-sm text-gray-600 mt-2">Age: {formData.age} years</p>}
                  </>
                ) : (
            ...
            <div className="space-y-3 md:space-y-4">
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-200 bg-gray-50">{formData.state || 'Not set'}</div>
                )}
              </div>
              <div>
                <label className="block text-xs md:text-sm font-semibold text-gray-700 mb-2">Geo-Political Zone</label>
                <div className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-200 bg-gray-50 text-gray-600">{formData.geo_zone || 'Not set'}</div>
              ...
              <div className={`flex items-center gap-3 p-3 md:p-4 rounded-lg ${isEditing ? 'bg-naija-green-50' : 'bg-gray-50 border border-gray-200'}`}>
                {isEditing ? (
                  <>
                    <input type="checkbox" name="physical_fitness" id="fitness" checked={formData.physical_fitness} onChange={handleChange} className="w-5 h-5 rounded border-gray-300 text-naija-green-600" />
                    <label htmlFor="fitness" className="text-xs md:text-sm text-gray-700">I am physically fit and ready for competition</label>
                  </>
                ) : (
                  <p className="text-xs md:text-sm text-gray-700">Physical Fitness: <span className="font-semibold">{formData.physical_fitness ? '✓ Yes' : '✗ No'}</span></p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-naija-green-100 p-4 md:p-6">
            ...
            <div className="space-y-3 md:space-y-4">
              <div>
                ...
                {isEditing ? (
                  <input type="text" name="emergency_contact" value={formData.emergency_contact} onChange={handleChange} placeholder="Full name"
                    className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-300 focus:outline-none focus:border-naija-green-600 focus:ring-2 focus:ring-naija-green-100" />
                ) : (
                  <div className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-200 bg-gray-50">{formData.emergency_contact || 'Not set'}</div>
                )}
              </div>
              <div>
                ...
                {isEditing ? (
                  <>
                    <input type="tel" name="emergency_phone" value={formData.emergency_phone} onChange={handleChange} placeholder="e.g., 08012345678 or +2348012345678"
                    ...
                    {emergencyPhoneError && <p className="text-xs text-red-600 mt-1 flex items-center gap-1"><AlertCircle size={12} />{emergencyPhoneError}</p>}
                  </>
                ) : (
                  <div className="w-full px-3 md:px-4 py-2 md:py-3 text-sm md:text-base rounded-lg border border-gray-200 bg-gray-50">{formData.emergency_phone || 'Not set'}</div>
                )}
              </div>
            </div>
          </div>
          ...
          {isEditing && (
            <div className="flex flex-col sm:flex-row gap-3 pt-4">
              <button onClick={handleCancelEdit} disabled={saving}
                className="flex-1 px-4 md:px-6 py-2 md:py-3 text-sm md:text-base rounded-lg border-2 border-gray-300 text-gray-700 font-semibold hover:bg-gray-50 transition flex items-center justify-center gap-2">
                <X size={16} /> Cancel
              </button>
              <button onClick={handleSubmit} disabled={saving || !!phoneError || !!emergencyPhoneError}
