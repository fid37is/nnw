...
export default function UserPaymentPage() {
  ...
  const loadUserApplication = async () => {
    try {
      ...
      if (appData) setApplicationId(appData.id)
    } catch (err) {
  ...
  const handlePaystackPayment = () => {
    if (!applicationId || !user) { toast.error('Application not found'); return }
    if (!window.PaystackPop) { toast.error('Payment system not loaded. Please refresh the page.'); return }

    const handler = window.PaystackPop.setup({
      key: process.env.NEXT_PUBLIC_PAYSTACK_PUBLIC_KEY,
      email: user.email,
      amount: config.payment_amount * 100,
      currency: 'NGN',
      ref: `NAIJA-${Date.now()}`,
      metadata: {
        custom_fields: [
          { display_name: 'Participant Name', variable_name: 'participant_name', value: user.full_name },
  ...
  const handleTransferSubmit = async () => {
    ...
    if (!applicationId) { toast.error('Application not found'); return }
        ...
        <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-green-100">
          <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2">
              ...
              <span className="font-bold text-lg text-green-900">Naija Next</span>
            </Link>
          </div>
        </nav>
        <div className="flex items-center justify-center min-h-[calc(100vh-80px)]">
          <div className="animate-spin w-12 h-12 border-4 border-green-200 border-t-green-600 rounded-full" />
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-green-50 to-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 md:py-12">
        <div className="text-center mb-8">
          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">Complete Your Payment</h1>
          <p className="text-gray-600">Choose your preferred payment method to complete your participation</p>
        </div>
          ...
          <div className="space-y-4">
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-sm text-gray-600 mb-2">Instant payment via debit/credit card. Automatically verified.</p>
                    <div className="flex items-center gap-2 text-xs text-green-600 font-semibold">
                      <CheckCircle className="w-4 h-4" /> Instant Confirmation
                    </div>
                  </div>
                </div>
              </button>
              ...
              >
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p className="text-sm text-gray-600 mb-2">Transfer to our bank account and upload payment proof.</p>
                    <div className="flex items-center gap-2 text-xs text-orange-600 font-semibold">
                      <AlertCircle className="w-4 h-4" /> Requires Verification (24-48 hours)
                    </div>
                  </div>
                </div>
              </button>
            ...
            {!config.paystack_enabled && !config.transfer_enabled && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 text-center">
                ...
                <p className="text-yellow-800 font-semibold">Payment methods are currently unavailable. Please contact support.</p>
              </div>
            )}
          </div>
        ) : paymentMethod === 'paystack' ? (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <button onClick={() => setPaymentMethod(null)} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6 transition text-sm">
            ...
            >
              {submitting ? <><Loader2 className="w-5 h-5 animate-spin" /> Processing...</> : <><CreditCard className="w-5 h-5" /> Proceed to Payment</>}
            </button>
            <p className="text-xs text-gray-500 text-center mt-4">🔒 Secured by Paystack - Your payment information is encrypted</p>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-lg p-8">
            <button onClick={() => setPaymentMethod(null)} className="flex items-center gap-2 text-gray-600 hover:text-gray-900 mb-6 transition text-sm">
            ...
            <div className="bg-gradient-to-br from-green-50 to-white border border-green-200 rounded-lg p-6 mb-6">
              ...
              <div className="space-y-3">
                {[
                  { label: 'Bank Name', value: config.bank_name, key: 'bank' },
                  { label: 'Account Number', value: config.account_number, key: 'account' },
                  { label: 'Account Name', value: config.account_name, key: 'name' },
                  { label: 'Amount', value: formatCurrency(config.payment_amount), key: 'amount', highlight: true },
                ].map(item => (
                  <div key={item.key} className="flex justify-between items-center p-3 bg-white rounded-lg">
                    <div>
                      <p className="text-xs text-gray-500">{item.label}</p>
                      <p className={`font-bold ${item.highlight ? 'text-green-700 text-lg' : 'text-gray-900'}`}>{item.value}</p>
                    </div>
                    <button
                      onClick={() => copyToClipboard(item.value, item.key)}
                      className="p-2 hover:bg-gray-100 rounded-lg transition"
                    >
                      {copied === item.key ? <Check className="w-4 h-4 text-green-600" /> : <Copy className="w-4 h-4 text-gray-400" />}
                    </button>
                  </div>
                ))}
              </div>
            ...
            >
              {submitting ? <><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</> : <><CheckCircle className="w-5 h-5" /> Submit Payment Proof</>}
            </button>
            <p className="text-xs text-gray-500 text-center mt-4">Your payment will be verified by our team within 24-48 hours</p>
          </div>
        )}
      </div>
    </div>
