    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 sticky top-24">
        ...
        {socialLinks.map((social) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 group-hover:text-naija-green-700 transition-colors">{social.name}</p>
              <p className="text-xs text-gray-500 truncate">{social.handle}</p>
            </div>
            <svg className="w-4 h-4 text-gray-400 group-hover:text-naija-green-600 group-hover:translate-x-0.5 transition-all flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="mt-5 pt-4 border-t border-gray-100">
        <div className="flex items-start gap-2 text-xs text-gray-600">
          ...
          <p className="leading-relaxed">Turn on notifications to stay updated with announcements and competition details.</p>
        </div>
      </div>
    </div>
  )
...
export default function UserDashboard() {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (seasonData) {
          ...
          setApplicationOpen(today >= startDate && today <= endDate)
        }
  ...
  const deleteMessage = async (notificationId: string) => {
    ...
    try {
      ...
      if (selectedMessage?.id === notificationId) setSelectedMessage(null)
  ...
  const deleteAllMessages = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return
      await supabase.from('user_notifications').delete().eq('user_id', session.user.id)
      setNotifications([])
    ...
    <main className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-12">
        ...
        <div className="flex gap-4 mb-6 border-b border-gray-200 overflow-x-auto">
          {[
            { key: 'application', icon: <FileText size={18} />, label: 'Application' },
            { key: 'inbox', icon: <MessageSquare size={18} />, label: 'Inbox', badge: unreadCount },
            { key: 'preferences', icon: <Settings size={18} />, label: 'Preferences' },
          ].map(tab => (
            <button
              key={tab.key}
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {tab.badge ? (
                  <span className="ml-1 bg-red-600 text-white text-xs font-bold px-2 py-0.5 rounded-full">{tab.badge}</span>
                ) : null}
              </div>
            </button>
          ))}
            ...
            <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-gradient-to-r from-green-50 to-green-100 border-2 border-green-300 rounded-xl p-6 shadow-lg">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-xl font-bold text-green-900 mb-2">Congratulations! You've Been Accepted!</h3>
              ...
              {application?.is_accepted && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm">
                  <p className="text-gray-700">
                    ...
                    <Link href="/user/profile" className="text-blue-600 underline font-semibold">profile page</Link>
                ...
                <>
                  <div className="bg-white rounded-2xl shadow-md border border-naija-green-100 p-6">
                    ...
                    <div className={`flex items-center gap-4 p-4 rounded-lg border-2 ${getStatusColor(application.status)}`}>
                      ...
                      <div>
                        ...
                        <p className="text-sm">Submitted on {new Date(application.submission_date).toLocaleDateString()}</p>
                      </div>
                    </div>
                    ...
                    <div className="mt-6 space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">{application.video_url ? '✓ Yes' : '✗ No'}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Photo Uploaded:</span>
                        <span className="font-semibold">{application.photo_url ? '✓ Yes' : '✗ No'}</span>
                      </div>
                      {application.is_accepted && (
                        <>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className={`font-semibold ${
                              ...
                              application.payment_status === 'pending' ? 'text-orange-600' : 'text-red-600'
                            }`}>
                              {application.payment_status === 'confirmed' ? '✓ Confirmed' :
                               application.payment_status === 'pending' ? '⏳ Pending Review' : '✗ Unpaid'}
                            </span>
                          </div>
                        </>
                      )}
                  ...
                  {(application.status === 'pending' || !application.video_url || !application.photo_url) ? (
                    ...
                    <div className="bg-naija-green-50 border-2 border-naija-green-200 rounded-lg p-3 text-center">
                      <p className="text-naija-green-900 font-semibold text-sm">✓ Application complete! Under review.</p>
                    </div>
                  ) : null}
                </>
              ) : (
                <div className="bg-white rounded-xl shadow-sm border border-naija-green-100 p-6 text-center">
                  ...
                  {checkingApplicationWindow ? (
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mt-2">
                      ...
                      <p className="text-xs text-gray-500 mt-1">Applications are not open at this time. Check back during the application window.</p>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="bg-naija-green-50 border border-naija-green-200 rounded-lg p-4">
                ...
                <ul className="space-y-2 text-xs text-gray-700">
                  {[
                    { label: 'Profile Photo', done: !!application?.photo_url },
                    { label: '2-3 Min Video', done: !!application?.video_url },
                    { label: 'Under Review', done: application?.status === 'under_review' || application?.status === 'approved' },
                    { label: 'Accepted', done: !!application?.is_accepted },
                    { label: 'Payment Confirmed', done: application?.payment_status === 'confirmed' },
                    { label: 'Participant Status', done: !!application?.is_participant },
                  ].map(item => (
                    <li key={item.label} className="flex items-center gap-2">
                      <span className="w-4 text-center font-bold">{item.done ? '✓' : '○'}</span>
                      {item.label}
                    </li>
                  ))}
                </ul>
        ...
        {activeTab === 'inbox' && (
          <div className="flex flex-col lg:grid lg:grid-cols-3 gap-6 h-auto lg:h-[calc(100vh-200px)]">
            <div className={`lg:col-span-1 ${selectedMessage ? 'hidden lg:block' : 'block'}`}>
              <div className="bg-white rounded-lg shadow-sm border border-naija-green-100 h-full flex flex-col">
                        ...
                        {notifications.map((notif: any) => (
                          ...
                          >
                            <div className="flex items-start gap-2">
                              ...
                              {!notif.is_read && <div className="w-2 h-2 bg-naija-green-600 rounded-full flex-shrink-0 mt-1.5" />}
                            </div>
                          </button>
                        ))}
                      </div>
            ...
            <div className={`lg:col-span-2 ${selectedMessage ? 'block' : 'hidden lg:block'}`}>
              <div className="bg-white rounded-lg shadow-sm border border-naija-green-100 p-4 sm:p-6 h-full flex flex-col">
                {selectedMessage ? (
                  <div className="flex flex-col h-full">
                    <div className="flex items-start justify-between mb-4 pb-4 border-b border-gray-200 flex-shrink-0 gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs sm:text-sm text-gray-600 mt-2">{new Date(selectedMessage.created_at).toLocaleString()}</p>
                      ...
                      <div className="flex gap-2 flex-shrink-0">
                        <button onClick={() => setSelectedMessage(null)} className="lg:hidden p-2 hover:bg-gray-100 text-gray-700 rounded-lg transition">
                          <X size={20} />
                        </button>
                        <button
                    ...
                    {!selectedMessage.is_read && (
                      <div className="text-xs text-gray-500 text-center flex-shrink-0">✓ Message marked as read</div>
                    )}
                  </div>
                ) : (
        ...
        {activeTab === 'preferences' && (
          <div className="bg-white rounded-lg shadow-sm border border-naija-green-100 p-4 sm:p-6 max-w-2xl">
            ...
            <div className="space-y-6">
              {[
                { key: 'in_app_notifications', label: 'In-App Notifications', desc: 'Receive notifications within the app' },
                { key: 'email_notifications', label: 'Email Notifications', desc: 'Receive updates via email' },
                { key: 'whatsapp_notifications', label: 'WhatsApp Notifications', desc: 'Receive messages on WhatsApp' },
              ].map(pref => (
                <div key={pref.key}>
                  <label className="flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={preferences[pref.key as keyof Preferences] as boolean}
                      onChange={e => setPreferences({ ...preferences, [pref.key]: e.target.checked })}
                      className="w-4 h-4 rounded border-gray-300"
                    />
                    <span className="ml-3">
                      <span className="block text-sm font-semibold text-gray-900">{pref.label}</span>
                      <span className="block text-xs text-gray-600">{pref.desc}</span>
                    </span>
                  </label>

                  {pref.key === 'whatsapp_notifications' && preferences.whatsapp_notifications && (
