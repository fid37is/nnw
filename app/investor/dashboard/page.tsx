    ...
    <div className="flex">
      ...
      <main className="flex-1 lg:ml-64 min-h-screen bg-gradient-to-br from-white via-naija-green-50 to-white">
        <div className="mx-auto px-4 py-8 lg:p-8">
          ...
          {/* Investment banner */}
          <Bone className="h-28 w-full rounded-xl mb-8" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
            {[0, 1].map(i => (
              <div key={i} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
                <Bone className="h-5 w-36" />
                {[0, 1, 2, 3, 4].map(j => (
                  <div key={j} className="flex justify-between py-1 border-b border-gray-100">
                    <Bone className="h-4 w-24" />
...
export default function InvestorDashboard() {
  ...
  const loadAll = async (isRefresh = false) => {
    ...
    try {
      ...
      const financialData = financialRows?.[0] ?? null
      ...
      const seasonResult = { data: seasonRows.data?.[0] ?? null }

      const apps = appsResult.data || []
      const activeZones = [...new Set(
        apps.filter(a => a.status === 'approved').map((a: any) => a.geo_zone)
      )].filter(Boolean)
  ...
  // Show skeleton while auth is being verified OR data is loading
                      ...
                      return (
                        <div key={i}>
                          ...
                          <div className="w-full bg-gray-100 rounded-full h-2">
                            <div className={`${stream.color} h-2 rounded-full transition-all duration-500`} style={{ width: `${pct}%` }} />
                          </div>
                        </div>
                      )
                    })}
